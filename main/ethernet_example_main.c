...
void eth_test()
{
    ESP_LOGI(TAG, "Request Task Is created");
    vTaskDelay(5000 / portTICK_PERIOD_MS);
    while(1)
    {
        if (s_ethernet_is_connected)
        {
            ESP_LOGI(TAG, "GOING TO GET DATA");
            if (HttpGet(url_link, data_buffer, 2048, NULL))
            {
                ESP_LOGI(TAG, "DATA RECEIVED : %s", data_buffer);
            }
        }
        else
        {
            ESP_LOGI(TAG, "LINK IS DOWN UNABLE TO SEND REQUEST");
        }
